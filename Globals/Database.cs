        ...
        internal static Error Create()
        {
            try
            {
                using (var connMetadata = new SQLiteConnection($"Data Source={Path.Combine(Settings.GetMetadataPath(), "metadata.db")}"))
                {
                    ...
                    cmd.CommandText = "CREATE TABLE IF NOT EXISTS common(sha TEXT PK, size INT, create INT, lastw INT, fstup INT, laste INT, type INT)";
                    cmd.ExecuteNonQuery();
                    ...
                    cmd.CommandText = "CREATE TABLE IF NOT EXISTS paths(sha TEXT FK, folder TEXT, file TEXT, PK (sha, folder, file))";
                    cmd.ExecuteNonQuery();
                    ...
                    cmd.CommandText = "CREATE TABLE IF NOT EXISTS tags(sha TEXT FK, tagId INT FK, PK (sha, tagId))";
                    cmd.ExecuteNonQuery();
                    ...
                    cmd.CommandText = "CREATE TABLE IF NOT EXISTS image(sha TEXT PK, avg INT, colr INT, dif INT, perc INT, wav INT, r INT, g INT, b INT, y INT, c INT, f INT, v INT, n INT, d INT, l INT, m INT, INT k, INT, a INT)";
                    cmd.ExecuteNonQuery();
                    ...
                    cmd.CommandText = "CREATE TABLE IF NOT EXISTS imports(id INT, sha TEXT, PK (id, sha));";
                    cmd.ExecuteNonQuery();
                }
                return Error.OK;
            }
