        ...
        internal static void Create()
        {
            try
            {
                using (var connMetadata = new SQLiteConnection($"Data Source={Path.Combine(Settings.GetMetadataPath(), "metadata.db")}"))
                {
                    ...
                    cmd.CommandText = "CREATE TABLE IF NOT EXISTS common (hash TEXT PK, size INT, creat INT, lastw INT, fstup INT, cat INT);";
                    cmd.ExecuteNonQuery();
                    ...
                    cmd.CommandText = "CREATE TABLE IF NOT EXISTS paths (hash TEXT FK REFERENCES common(hash), folder TEXT, file TEXT, PRIMARY KEY (hash, folder, file));";
                    cmd.ExecuteNonQuery();
                    ...
                    cmd.CommandText = "CREATE TABLE IF NOT EXISTS tags (hash TEXT FK REFERENCES common(hash), tagId INT FK, PRIMARY KEY (hash, tagId));";
                    cmd.ExecuteNonQuery();
                    ...
                    cmd.CommandText = "CREATE TABLE IF NOT EXISTS image (hash TEXT FK REFERENCES common(hash), type INT, avg INT, colr INT, dif INT, perc INT, wav INT, r INT, g INT, b INT, y INT, c INT, f INT, v INT, n INT, d INT, l INT, m INT, k INT, a INT);";
                    cmd.ExecuteNonQuery();
                    cmd.CommandText = "CREATE INDEX IF NOT EXISTS _hash ON image(hash);";
                    cmd.ExecuteNonQuery();

                    // id comes first in PK because that will be what I query by
                    // a FK for id would require renaming imports.paths/imports and moving them into this database (or renaming the other ones obviously)
                    cmd.CommandText = "CREATE TABLE IF NOT EXISTS imports (id INT, hash TEXT FK REFERENCES common(hash), PRIMARY KEY (id, hash));";
                    cmd.ExecuteNonQuery();
